...
3. Apply Alembic revision 019, then let the designated legacy owner complete
   ...
   account exactly once. Apply revision 021 only after that login; it aborts
   if the nominated email resolves to anything other than exactly one account
   or if any account-owned orphan remains.
