<script lang="ts">
  ...
  import { isAuthenticated, aliasRequired, checkAuth, googleLogin } from '$lib/auth';
  ...
  async function handleGoogleLogin() {
    ...
    try {
      await googleLogin();
      if (!isNative()) return;
      if ($isAuthenticated) await goto($aliasRequired ? '/onboarding/alias' : '/');
      else loginError = 'Sitzung konnte nicht geprüft werden. Bitte erneut versuchen.';
    } catch (error) {
      if ((error as { code?: string })?.code !== 'LOGIN_CANCELLED')
        loginError = error instanceof Error ? error.message : 'Anmeldung fehlgeschlagen. Bitte erneut versuchen.';
    } finally {
      signingIn = false;
...
<div class="login">
  <div class="login-card">
    ...
    {:else}
      <button bind:this={loginButton} class="g-btn" onclick={handleGoogleLogin} disabled={signingIn} aria-busy={signingIn}>
        ...
        <span>{signingIn ? 'Anmeldung läuft …' : 'Mit Google anmelden'}</span>
      </button>
    {/if}
  </div>
</div>
