<script lang="ts">
  ...
  let privateNotesExpanded = false;

  $: privateNotes = notes.filter((note) => !note.space_id && note.status === 'active');
  $: visiblePrivateNotes = privateNotesExpanded ? privateNotes : privateNotes.slice(0, 9);
  $: hiddenPrivateNoteCount = Math.max(0, privateNotes.length - visiblePrivateNotes.length);
  ...
  function openDetails(note: Note, element?: HTMLElement) {
    ...
    void tick().then(() => { if (!detailDialog.open) detailDialog.showModal(); detailDialog.querySelector<HTMLButtonElement>('.close')?.focus(); });
  }
  ...
  async function move(note: Note, spaceId: string | null) {
    if (saving || note.space_id === spaceId) return;
    const movingPrivate = spaceId === null;
    const message = movingPrivate
      ? 'Die Notiz wird wieder privat. Bereichsmitglieder verlieren den Zugriff; ein offenes, daraus geplantes To-do wird ebenfalls privat.'
      : `Die Notiz wird für alle Mitglieder von ${spaces.find((space) => space.id === spaceId)?.name ?? 'diesem Bereich'} sichtbar.`;
    if (!window.confirm(message)) return;
    saving = true;
    const result = await api.moveNote(note.id, spaceId, !movingPrivate, movingPrivate);
    saving = false;
  ...
  async function dropPrivate(event: DragEvent) { const item = droppedItem(event); const note = notes.find((value) => item === `note:${value.id}`); if (note) await move(note, null); draggedId = ''; draggedTodoId = ''; }
  ...
  async function finishTouchDrag(event: PointerEvent) {
    ...
    const target = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>('[data-area-id],[data-private],[data-slot-date],[data-timeline],[data-untimed]');
    touchDrag = null;
    ...
    else if (target.dataset.private !== undefined) await move(note, null);
...
<style>
  ...
  @media(max-width:620px){.grid,.planning-notes,.area-notes{grid-template-columns:repeat(4,minmax(0,1fr))}.note{min-height:0}.board-header h2{font-size:18px}}
