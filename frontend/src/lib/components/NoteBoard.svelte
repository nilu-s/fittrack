...
{#if open}
  <section id="note-board" class="board" aria-label="Notiz-Board">
    ...
    {:else}<section aria-labelledby="inbox-title"><div class="section-title"><h3 id="inbox-title">Eingang</h3><span>{privateNotes.length} privat</span></div><div class="grid">{#each privateNotes.slice(0, 9) as note (note.id)}<button class="note" type="button" draggable="true" ondragstart={(event) => dragStart(event, note)} onclick={(event) => openDetails(note, event.currentTarget)}><strong>{note.title}</strong>{#if note.body}<small>{note.body}</small>{/if}</button>{:else}<p class="empty">Noch keine privaten Notizen.</p>{/each}</div></section>
    <section aria-labelledby="areas-title"><div class="section-title"><h3 id="areas-title">Bereiche</h3><span>Ablage und Zugriff</span></div><div class="areas">{#each spaces as space (space.id)}<div class="area" role="group" aria-label={`Bereich ${space.name}; Notiz hier ablegen`} ondragover={allowDrop} ondrop={(event) => dropArea(event, space.id)}><div><strong>{space.name}</strong><small>{space.members.length} Mitglieder</small></div><div class="area-notes">{#each notes.filter((note) => note.space_id === space.id && note.status === 'active') as note (note.id)}<button class="note compact" type="button" draggable="true" ondragstart={(event) => dragStart(event, note)} onclick={(event) => openDetails(note, event.currentTarget)}>{note.title}</button>{:else}<span>Notiz hier ablegen oder eine Kachel öffnen und den Bereich wählen.</span>{/each}</div></div>{:else}<p class="empty">Lege unter Einstellungen einen Bereich an, um Notizen zu teilen.</p>{/each}</div></section>
    <section aria-labelledby="calendar-title"><div class="section-title"><h3 id="calendar-title">Planen</h3><span>Tag oder Uhrzeit ablegen</span></div><div class="calendar">{#each days as day (dateValue(day))}<div class="day"><strong>{dateLabel(day)}</strong><button type="button" class="all-day" ondragover={allowDrop} ondrop={(event) => dropSlot(event, dateValue(day))} onclick={() => boardMessage = 'Für Tastatur und Touch: Öffne eine Kachel und wähle dort Datum und Uhrzeit.'}>Ganztägig</button>{#each ['09:00', '13:00', '17:00'] as time}<button type="button" class="slot" ondragover={allowDrop} ondrop={(event) => dropSlot(event, dateValue(day), time)} onclick={() => boardMessage = 'Für Tastatur und Touch: Öffne eine Kachel und wähle dort Datum und Uhrzeit.'}>{time}</button>{/each}</div>{/each}</div></section>
