...
<div class="td slide-down">
  ...
  {:else}
    ...
    <div class="ex-card">
      ...
      {#each sets as set, idx (idx)}<div class="set-row"><span class="set-n">{idx + 1}</span><select aria-label={`Satztyp ${idx + 1}`} value={set.set_type ?? 'work'} onchange={(e) => updateSet(ex.exercise_name, idx, 'set_type', (e.target as HTMLSelectElement).value as SetType)}><option value="warmup">W</option><option value="work">A</option><option value="top">★</option><option value="backoff">B</option><option value="drop">D</option></select><input aria-label={`Wiederholungen Satz ${idx + 1}`} min="0" step="1" type="number" placeholder="—" value={set.reps ?? ''} oninput={(e) => updateSet(ex.exercise_name, idx, 'reps', ((e.target as HTMLInputElement).value === '' ? null : Number((e.target as HTMLInputElement).value)))} /><input aria-label={`Gewicht in kg Satz ${idx + 1}`} min="0" step="0.25" type="number" placeholder="—" value={set.weight_kg ?? ''} oninput={(e) => updateSet(ex.exercise_name, idx, 'weight_kg', ((e.target as HTMLInputElement).value === '' ? null : Number((e.target as HTMLInputElement).value)))} /><input aria-label={`RIR Satz ${idx + 1}`} min="0" max="10" step="1" type="number" placeholder="—" value={set.rir ?? ''} oninput={(e) => updateSet(ex.exercise_name, idx, 'rir', ((e.target as HTMLInputElement).value === '' ? null : Number((e.target as HTMLInputElement).value)))} /><button class="set-remove" onclick={() => removeSet(ex.exercise_name, idx)} disabled={sets.length <= 1} aria-label={`Satz ${idx + 1} entfernen`}>×</button></div>{/each}
