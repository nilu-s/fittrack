<script lang="ts">
  ...
  $: if (item && dialog && !dialog.open) {
    ...
    note = item.note ?? '';
    dialog.showModal();
    tick().then(() => dialog.querySelector<HTMLButtonElement>('header button')?.focus());
  }
  ...
  function save() {
    ...
    const parsed = quantity.trim() ? Number(quantity) : null;
...
<dialog bind:this={dialog} class="editor" aria-labelledby="shopping-editor-title" oncancel={(event) => { event.preventDefault(); close(); }} onclose={() => opener?.focus()}>
  {#if item}
    <form onsubmit={(event) => { event.preventDefault(); save(); }}>
      <header><div><p>ARTIKEL</p><h2 id="shopping-editor-title">Einkauf bearbeiten</h2></div><button type="button" onclick={close} aria-label="Bearbeiten schließen"><Icon name="x" size={20}/></button></header>
...
<style>
  .editor { width:min(calc(100% - 24px),460px); margin:auto; padding:0; border:1px solid var(--border-default); border-radius:var(--radius-modal); background:var(--surface-default); color:var(--text-primary); box-shadow:var(--shadow-modal); }
  .editor::backdrop { background:var(--overlay-backdrop); }
  form { display:grid; gap:var(--space-3); padding:var(--space-4); }
  header { display:flex; justify-content:space-between; gap:var(--space-3); }
  header p { margin:0 0 3px; color:var(--text-tertiary); font-size:11px; font-weight:700; letter-spacing:.06em; }
  h2 { margin:0; font-size:18px; }
  label { display:grid; gap:5px; color:var(--text-secondary); font-size:12px; }
  input { width:100%; min-height:var(--control-min); padding:8px 10px; border:1px solid var(--border-default); border-radius:var(--radius-control); background:var(--surface-raised); color:var(--text-primary); font:inherit; }
