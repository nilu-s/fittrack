...
<footer class="day-footer" aria-label="Tagesaktionen">
  <form class="todo-add" onsubmit={(event) => { event.preventDefault(); shoppingOpen ? submitShopping() : generalTodoOpen ? submitGeneralTodo() : submitTodo(); }}>
    <label class="sr-only" for="footer-entry-title">{shoppingOpen ? 'Artikel suchen oder hinzufügen' : generalTodoOpen ? 'Private Notiz hinzufügen' : 'Private Notiz hinzufügen'}</label>
    <input id="footer-entry-title" placeholder={shoppingOpen ? 'Artikel suchen oder hinzufügen…' : '+ Notiz festhalten…'} value={shoppingOpen ? shoppingTitle : generalTodoOpen ? generalTodoTitle : todoTitle} oninput={updateFooterEntry} disabled={shoppingOpen ? shoppingAdding : generalTodoOpen ? generalTodoAdding : todoAdding} aria-describedby={!shoppingOpen && !generalTodoOpen && todoAddError ? 'footer-todo-error' : undefined} />
    <button type="submit" disabled={shoppingOpen ? !shoppingTitle.trim() || shoppingAdding : generalTodoOpen ? !generalTodoTitle.trim() || generalTodoAdding : !todoTitle.trim() || todoAdding} aria-label={shoppingOpen ? 'Artikel hinzufügen' : 'Notiz hinzufügen'}><Icon name="plus" size={16} /></button>
  ...
  <div class="dnav" role="slider" aria-label="Horizontal wischen zum Wechseln" aria-valuemin="-1" aria-valuemax="1" aria-valuenow="0" aria-valuetext={`${dow}, ${dateLabel}`} tabindex="0" onkeydown={onNavigationKeydown} ontouchstart={onTouchStart} ontouchend={onTouchEnd}>
    ...
    <button type="button" class:active={generalTodoOpen} class="drawer-zone todo-toggle" aria-controls="note-board" aria-expanded={generalTodoOpen} aria-label="Privates Notiz-Board öffnen" onclick={() => dispatch('generaltodogesture', 140)} onkeydown={openGeneralTodoWithKeyboard} onpointerdown={startGeneralTodoGesture} onpointermove={moveGeneralTodoGesture} onpointerup={endGeneralTodoGesture} onpointercancel={endGeneralTodoGesture}><span class="drawer-grip" aria-hidden="true"></span><span>Notizen</span><span class="todo-count">{generalTodoCount}</span></button>
