<script lang="ts">
  ...
  function openPicker() { pickerMonth = new Date(`${$currentDate}T00:00:00`); pickerDialog.showModal(); }
  function closePicker() { pickerDialog.close(); }
  function selectDate(date: string) { currentDate.set(date); pickerDialog.close(); }
...
<footer class="day-footer" aria-label="Tagesaktionen">
  ...
  <nav class="dnav" aria-label="Tagesnavigation" ontouchstart={onTouchStart} ontouchend={onTouchEnd}>
    ...
    <button class="dnav-mid" onclick={(e) => guarded(goToday, e)} aria-label="Heute"><span class="dnav-date">{dow}, {dateLabel}</span><span class="dnav-today">{isToday ? 'Heute' : 'Zu heute'}</span></button>
...
<dialog bind:this={pickerDialog} class="date-picker" aria-labelledby="date-picker-title" onclose={restoreCalendarFocus}>
  <div class="picker-content">
    ...
    <div class="picker-actions"><button type="button" class="picker-secondary" onclick={() => selectDate(todayStr())}>Heute</button><span></span><button type="button" class="picker-secondary" onclick={closePicker}>Abbrechen</button></div>
  </div>
</dialog>

<style>
  .day-footer { position:fixed; z-index:40; left:50%; bottom:max(10px, env(safe-area-inset-bottom, 0px)); transform:translateX(-50%); display:grid; gap:6px; width:min(calc(100% - 20px), 460px); }
  ...
  @media (max-width:380px) { .dnav { grid-template-columns:44px minmax(0,1fr) 44px 44px; } .dnav-step { width:44px; padding:0; } .dnav-step span { display:none; } }
</style>
