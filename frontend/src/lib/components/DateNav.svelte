<script lang="ts">
  ...
  $: entryPlaceholder = entryMode === 'shopping' ? 'Artikel suchen oder hinzufügen…' : entryMode === 'note' ? noteTargetName ? `Notiz für ${noteTargetName} festhalten…` : 'Notiz festhalten…' : isToday ? 'To-do für heute hinzufügen…' : `To-do für ${dateLabel} hinzufügen…`;
...
{#if calendarOpen}
<dialog use:modal class="calendar-overlay" aria-labelledby="date-picker-title" oncancel={(event) => { event.preventDefault(); closePicker(); }} onclick={(event) => { if (event.target === event.currentTarget) closePicker(); }}>
  <div class="date-picker">
    <div class="picker-content">
      ...
      <div class="calendar-days" role="group" aria-label={pickerMonthLabel}>{#each visibleCalendarDays as calendarDay (calendarDay.date)}<button type="button" class:outside-month={!calendarDay.inMonth} class:selected={calendarDay.date === $currentDate} class:today={calendarDay.date === todayStr()} onclick={() => selectDate(calendarDay.date)} aria-label={calendarDay.date} aria-pressed={calendarDay.date === $currentDate} aria-current={calendarDay.date === todayStr() ? 'date' : undefined}>{calendarDay.day}</button>{/each}</div>
      <div class="picker-actions"><button data-dialog-close type="button" class="picker-secondary" onclick={closePicker}>Schließen</button><button type="button" class="picker-secondary" onclick={() => selectDate(todayStr())}>Heute</button></div>
    </div>
  </div>
</dialog>
{/if}
