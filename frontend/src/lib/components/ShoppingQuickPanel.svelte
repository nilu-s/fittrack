<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{ close: void; toggle: ShoppingItem; remove: ShoppingItem; edit: ShoppingItem; import: number; choose: string; resize: number }>();
  ...
  function endResize() { if (dragOffset >= 88) dispatch('close'); dragOffset = 0; resizeStartY = 0; }
...
{#if open}
  <aside id="shopping-quick-panel" class="panel" style={`--panel-height: ${panelHeight}px; --drag-offset: ${dragOffset}px`} aria-label="Einkaufsliste">
