<script lang="ts">
  ...
  function updateVisualViewport() {
    ...
    document.documentElement.style.setProperty('--shopping-search-viewport-height', `${viewport?.height ?? window.innerHeight}px`);
    document.documentElement.style.setProperty('--shopping-search-viewport-top', `${viewport?.offsetTop ?? 0}px`);
  }
  ...
  function finishSheetDrag(event: PointerEvent) {
    if (dragStartY !== null && event.clientY - dragStartY > 56) close();
    dragStartY = null;
  }
...
{#if open}
  <aside id="shopping-quick-panel" class="panel" class:inline class:searchActive aria-label="Einkaufsliste" in:fly={panelEnter()} out:fly={panelExit()}>
    {#if searchActive}
      ...
      <button class="search-backdrop" type="button" aria-label="Suche schließen" onclick={close}></button>
      <section class="search-sheet" aria-labelledby="shopping-search-results">
        <button class="sheet-handle" type="button" aria-label="Suche schließen" onpointerdown={beginSheetDrag} onpointerup={finishSheetDrag} onclick={close}><span aria-hidden="true"></span></button>
