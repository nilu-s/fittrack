<script lang="ts">
  ...
  import type { Meal, PhotoAnalysisResponse } from '$lib/types';
  ...
  async function onStandalonePhotoSelected(e: Event) {
    ...
    photoLoading = true;
    try {
      const result = await api.analyzePhoto(file) as PhotoAnalysisResponse | null;
      const parsed = parseVisionResult(result);
      if (parsed) {
        confirmData = { slot: getCurrentSlot(), ...parsed, matched: result?.dish_match?.matched ?? false };
        choosingSlot = false;
      }
    } catch {} finally { photoLoading = false; input.value = ''; }
  }
