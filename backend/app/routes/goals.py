...
async def _get_goal_map(session, account_id) -> dict[str, Goal]:
...
async def _resolve_goal_value(key: str, session, account_id) -> Decimal:
    """Return persisted value for a key, falling back to DEFAULT_GOALS."""
    result = await session.execute(select(Goal).where(Goal.account_id == account_id, Goal.key == key))
...
async def list_goals(user: str = Depends(get_current_user)):
    ...
    async with async_session() as session:
        goals = await _get_goal_map(session, user)
...
async def get_goal(key: str, user: str = Depends(get_current_user)):
    async with async_session() as session:
        result = await session.execute(select(Goal).where(Goal.account_id == user, Goal.key == key))
        goal = result.scalar_one_or_none()
        if goal is None:
