...
`CRONICL_SCALE_DIAGNOSTIC_START` through the last `FRAME` line. The diagnostic
prints advertisement data, every discovered GATT service/characteristic and
all notification/indication frames; it never sends a measurement to Cronicl.
...
| `API_HOST` | Cronicl server hostname |
